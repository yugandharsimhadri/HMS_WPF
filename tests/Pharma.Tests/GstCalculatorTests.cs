...
public class GstCalculatorTests
{
    ...
    public void Tax_is_extracted_from_the_mrp_not_added_to_it()
    {
        ...
        var line = GstCalculator.Line(mrp: 112m, unitsPerPack: 1, quantity: 10, discountPercent: 0m, gstRate: 12m);
    ...
    {
        var line = GstCalculator.Line(mrp: 33.33m, unitsPerPack: 1, quantity: 3, discountPercent: 0m, gstRate: 5m);
    ...
    {
        var full = GstCalculator.Line(100m, 1, 1, 0m, 12m);
        var discounted = GstCalculator.Line(100m, 1, 1, 10m, 12m);
    ...
    {
        var lines = new[] { GstCalculator.Line(10.40m, 1, 1, 0m, 12m) };
    ...
    public void Round_off_always_reconciles_the_printed_total()
    {
        var lines = new[]
        {
            GstCalculator.Line(23.75m, 1, 3, 0m, 5m),
            GstCalculator.Line(112.50m, 1, 2, 7.5m, 12m),
            GstCalculator.Line(9.99m, 1, 7, 0m, 18m)
        };
    ...
    public void Taxable_plus_tax_equals_what_the_customer_pays()
    {
        var lines = new[]
        {
            GstCalculator.Line(45m, 1, 4, 5m, 12m),
            GstCalculator.Line(120m, 1, 1, 0m, 5m)
        };
